...
from compiler.engine.walk_nodes import walk_doc_nodes
...
def _parse_string(annotation_subtree: list[DocNode]) -> str:
    ...
    annotation_line_text = extract_inline_text(annotation_node.inline_children)
    ...
    # case 2. mult lines
    return "\n".join(
        extract_inline_text(value_node.inline_children)
...
def _parse_bool(annotation_subtree: list[DocNode]) -> bool:
    annotation_node = annotation_subtree[0]
    text = extract_inline_text(annotation_node.inline_children)
...
def parse_annotation(node: DocNode, rules: dict[str, AnnotationRule]) -> Annotation|None:
    ...
    annotation_subtree = list(walk_doc_nodes(node))
